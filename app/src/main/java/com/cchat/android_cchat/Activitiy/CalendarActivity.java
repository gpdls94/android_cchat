...
/// [0204] [0205] [0206] 이렇게 넣음
    ...
    /// 생성자
    public CalendarActivity(Context context, LinearLayout layout )
    {
/// context저장
        m_context = context ;
...
//        today_year = m_Calendar.get( Calendar.YEAR );
//        today_month = m_Calendar.get( Calendar.MONTH );
//        today_date = m_Calendar.get( Calendar.DATE );
...
/// 선택된 날짜칸에 변화를 주기위한 함수 1호
    ...
    {
        m_cellTextBtn[ today_cell ].setTextColor(m_context.getResources().getColor(R.color.cchat_text_color));
        m_cellTextBtn[ today_cell ].setBackgroundResource(android.R.color.transparent);

        if (m_Calendar.get( Calendar.MONTH ) == Calendar.getInstance().get(Calendar.MONTH)
                && m_Calendar.get( Calendar.YEAR ) == Calendar.getInstance().get(Calendar.YEAR)) {
...
/// 숫자 표시되는 부분의 넓이와 높이
                        LinearLayout.LayoutParams m_cellTextBtnParams = new LinearLayout.LayoutParams( m_cWidth, m_cHeight );
                        m_cellTextBtn[ cellnum ].setLayoutParams(m_cellTextBtnParams) ;
...
//int imonth =  iCal.get( Calendar.MONTH ) ;
            if(( m_Calendar.get( Calendar.MONTH ) + 1 ) < 10) {
                m_mothTv.setText( "0" + ( m_Calendar.get( Calendar.MONTH ) + 1 ) + "" ) ;
            } else {
